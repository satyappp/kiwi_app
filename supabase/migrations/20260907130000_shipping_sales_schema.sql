...
create policy shipping_sales_insert on public.shipping_sales for insert to authenticated with check (created_by = (select auth.uid()));
...
create policy shipping_sale_allocations_insert on public.shipping_sale_allocations for insert to authenticated with check (created_by = (select auth.uid()));
...
grant select, insert on table public.shipping_sales, public.shipping_sale_allocations to authenticated;
