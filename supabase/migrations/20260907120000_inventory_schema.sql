-- Inventory projection, reservations, and shipment records.
...
create policy inventory_shipments_select on public.inventory_shipments
  for select to authenticated using (true);
create policy inventory_shipments_insert on public.inventory_shipments
  for insert to authenticated with check (created_by = (select auth.uid()));
create policy inventory_shipments_update on public.inventory_shipments
  for update to authenticated using (true) with check (true);
